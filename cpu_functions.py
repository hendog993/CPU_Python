...
class Program:
	...
	def add(self):
		print(self.op_2)
		# Adds the values of registers A and B and stores in M.A. Op code 2
		self.RAM[int(self.op_2)] = self.REG[0] + self.REG[1]

	def sub(self):
		# Subtracts the values of registers c and d, stores in M.A. Op code 2
		self.RAM[int(self.op_2)] = self.REG[2] - self.REG[3]

	def grt(self):
		self.RAM[int(self.op_2)] = int(self.REG[0]) > int(self.REG[1])
	...
	def str(self):
		# Store the value in REG 1 in memory address op code 2.
		self.RAM[int(self.op_2)] = self.REG[0]
		pass
